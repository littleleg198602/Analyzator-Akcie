...
class MT5AnalyzerApp(tk.Tk):
    ...
    def _build_dashboard_tab(self) -> None:
        ...
        self.dashboard_scroll_canvas = tk.Canvas(self.dashboard_tab, highlightthickness=0)
        dashboard_scrollbar = ttk.Scrollbar(self.dashboard_tab, orient="vertical", command=self.dashboard_scroll_canvas.yview)
        ...
        parent = ttk.Frame(self.dashboard_scroll_canvas)
        self.dashboard_content = parent
        self.dashboard_window = self.dashboard_scroll_canvas.create_window((0, 0), window=parent, anchor="nw")
        parent.bind("<Configure>", lambda _event: self.dashboard_scroll_canvas.configure(scrollregion=self.dashboard_scroll_canvas.bbox("all")))
        self.dashboard_scroll_canvas.bind("<Configure>", lambda event: self.dashboard_scroll_canvas.itemconfigure(self.dashboard_window, width=event.width))
        self.dashboard_scroll_canvas.bind_all("<MouseWheel>", lambda event: self.dashboard_scroll_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units"))
        ...
        self.analysis_canvas.grid(row=5, column=0, sticky="nsew", padx=4, pady=4)
        self.profit_canvas = tk.Canvas(parent, height=280, bg="white")
        self.profit_canvas.grid(row=5, column=1, sticky="nsew", padx=4, pady=4)
